...
typedef struct {
    uint32_t restoreNum;
    OutRestoreEntry_t* outRestoreBase;
} OutRestore_t; // returned by the outside application for query
...
typedef struct {
    ...
    Recipe_t* outRecipe;
    Recipe_t* outSecureRecipe;
    Recipe_t* outKeyRecipe;
    OutQuery_t* outQuery;
    void* outClient;
    void* sgxClient;
} UpOutSGX_t;
...
typedef struct {
    uint32_t queryNum;
    outMigrationEntry_t* outMigrationBase;
} outMigration_t;
typedef struct {
    ReqContainer_t* reqContainer;
    SendMsgBuffer_t* sendChunkBuf;
    outMigration_t* outMigration;
    void* outClient;
    void* sgxClient;
} MrOutSGX_t;
