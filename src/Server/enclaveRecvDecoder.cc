...
void EnclaveRecvDecoder::Run(ClientVar* outClient) {
    ...
    tool::Logging(myName_.c_str(), "Download File Start.\n");
    ...
    uint8_t* readKeyRecipeBuf = outClient->_readKeyRecipeBuf;
    ...
    outClient->_keyRecipeReadHandler.read(tmp, sizeof(FileRecipeHead_t));
    free(tmp);
    ...
    while (!end) {
        ...
        outClient->_keyRecipeReadHandler.read((char*)readKeyRecipeBuf, 
            sizeof(keyRecipeEntry_t) * sendRecipeBatchSize_);
        ...
        Ecall_ProcRecipeBatch(eidSGX_, readRecipeBuf, readKeyRecipeBuf, recipeEntryNum, 
            resOutSGX);
    }
