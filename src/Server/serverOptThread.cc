...
ServerOptThread::ServerOptThread(SSLConnection* dataSecureChannel, 
    ...
    // init the migration
    enclaveMigrationObj_ = new EnclaveMigrator(eidSGX_, indexType_);
...
void ServerOptThread::Run(SSL* clientSSL, int migrateFileNum) {
    ...
    if(optType == UPLOAD_OPT)
        tool::Logging(myName_.c_str(), "Upload File Finished.\n");
    else if(optType == DOWNLOAD_OPT)
        tool::Logging(myName_.c_str(), "Download File Finished.\n");
    
    // *****************************
    // TODO: select upload file list
    // *****************************
    ...
    if(optType == UPLOAD_OPT)
    {
        ...
        if(uploadFileList.size() == migrateFileNum){
            //tool::Logging(myName_.c_str(),"m start\n");
            sleep(5);
            enclaveMigrationObj_->MigrateFileToCloud(uploadFileList);
            ...
            {
                enclaveMigrationObj_->DownloadRecipeFromCloud(uploadFileList[i]);
                enclaveMigrationObj_->DownloadChunkFromCloud(uploadFileList[i]);
            }
        }
